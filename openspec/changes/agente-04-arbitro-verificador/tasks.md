...
- [ ] 1.1 Restringir `BaseAgent.parse_hypotheses()` a una whitelist de campos al construir `Source` (`pmid`, `title`, `journal`, `year`, `url`), descartando en silencio `verified`, `verification_status`, `actual_title` y `publication_types` (D9). Verificar con un test en `tests/test_base_agent.py` que parsea una respuesta con `verified: true` y tipos de publicación inventados y comprueba que la `Source` resultante los tiene en `None`/vacío.
- [ ] 1.2 Verificar que la suite existente sigue verde tras el cambio de parseo: `pytest tests/ --ignore=tests/test_ingesta.py` (Tesseract no está instalado en la máquina de desarrollo).
...
- [ ] 5.4 Implementar las guardas de salida del modelo: descartar referencias que no estaban en la entrada, ignorar intentos de subir nivel y de descartar hipótesis, y contabilizar los descartes (spec `arbitro-consenso`). Verificar con tests dedicados para cada una de las tres guardas.
...
- [ ] 6.4 Implementar la re-verificación contra PubMed de las fuentes aceptadas, reutilizando `verification.py` sobre el subconjunto. Verificar con un test mockeado: título correcto → verificada y la hipótesis pasa a respaldada; título que no corresponde → discordante.
